...
let computeArea polygon =
    ...
    List.map2 partial polygon polygon2 |> List.sum |> (fun x -> x / 2L) |> abs

let computePerimeter plans =
    plans |> List.map _.Amount |> List.sum
...
let part2 plans = plans |> List.map reinterpret |> part1
...
let parsePlan (str: string) =
    ...
    | [| dir; amt; col |] ->
        { Direction = dir
          Amount = int64 amt
          Color = col[2 .. col.Length - 2] }
    | x -> failwith $"invalid plan {x}"

let main filename =
    let plans =
        filename |> System.IO.File.ReadAllLines |> Array.map parsePlan |> List.ofArray
